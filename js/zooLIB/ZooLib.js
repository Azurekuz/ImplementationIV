...
class Profile{
    ...
    die(){
        this.justDied = true;
        if(curPen.animalType === this){
            pushAlert("💀 The " + curPen.animalType.typeName + " have died.", "#ff8080");
            playProperly(sfx_church);
            this.alertedDeath();
        }
        this.alive = false;
    }
...
class ProfileManager{
    ...
    populate(){
        this.profiles.push(new Profile("Tigers", 7));
        this.profiles.push(new Profile("Lions", 10));
        this.profiles.push(new Profile("Zebras", 15));
        this.profiles.push(new Profile("Gazelles", 14));
        this.profiles.push(new Profile("Sea Lions", 11));
        this.profiles.push(new Profile("Penguins", 10));
        this.profiles.push(new Profile("Orangutans", 12));
        this.profiles.push(new Profile("Giraffes", 14));
        this.profiles.push(new Profile("Birds", 8));
        this.profiles.push(new Profile("Snakes", 13));
...
class Pen{
    ...
    canBeFed(curTime){
        if(this.isStarving(curTime) && this.animalType.alive){
            return true;
        }else{
            return false;
        }
    ...
    feed(){
        this.justFed = true;
        if(curPen == this){
            pushAlert("🐵 " + curPen.animalType.typeName + " have been fed!", "#caffb9");
    ...
    starveToDeath(curTime){
        if((this.lastFed-(this.animalType.getRecFeedingInterval()*2))>=0 && this.animalType.alive){
            this.animalType.die();
            return true;
        }
        return false;
    ...
    getLast(){
        if(this.animalType.alive){
            return this.lastFed + " hour(s) ago";
        }else{
            return "DEAD";
        }
    }
...
class Zone{
    ...
    getLastWater(){
        if(this.lastWatered==0){
            return "Moments ago"
        }
        return this.lastWatered + " hour(s) ago";
    }
    ...
    water(){
        if(this.isWatering && (this.lastWatered-this.setWaterInterval)>=0){
            this.lastWatered = 0;
            if(curZone.zoneName == this.zoneName){
                pushAlert("💧 Sprinklers have just went off for " + this.zoneName +"!", "#caffb9");
